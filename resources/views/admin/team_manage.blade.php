...
<form method="POST" action="/admin/equipes/{{ (int) $team['id'] }}/update" class="admin-form-stack admin-form-stack--wide">
    ...
    @else
            ...
            <table class="admin-table">
                ...
                @foreach ($members as $member)
                    <tr>
                        ...
                        <td>
                            <select class="form-control" name="members[{{ (int) $member['id'] }}][class]" style="min-width:130px;">
                                <option value="">—</option>
                                @foreach ($classes as $classKey => $classLabel)
                                    <option value="{{ $classKey }}" @selected((string) ($member['class'] ?? '') === $classKey)>{{ $classLabel }}</option>
                                @endforeach
                            </select>
                            <select class="form-control" name="members[{{ (int) $member['id'] }}][status]" style="min-width:120px; margin-top:6px;">
                        ...
                        <td>
                            <label style="cursor:pointer; white-space:nowrap; color:#ddd;">
                                <input type="checkbox" name="members[{{ (int) $member['id'] }}][is_leader]" value="1"
                                       @checked((bool) $member['is_leader']) style="margin-right:6px;">
                                Leader
                            </label>
                        </td>
                        <td>
                        ...
                        <td>
                            <button type="submit" form="member-remove-{{ (int) $member['id'] }}"
                                    class="btn-icon admin-btn--danger" title="Retirer du roster"
        ...
        <span class="form-hint">Les leaders peuvent éditer la page publique de l'équipe (description, logo, roster). Désignez au moins un leader avant l'activation.</span>
    @endif

    <div style="border-top:1px solid #2c3e50; padding-top:16px; display:flex; justify-content:flex-end;">
        <button type="submit" class="admin-btn admin-btn--primary">
            <i class="fa-solid fa-floppy-disk"></i> Enregistrer les changements
        </button>
    </div>
</form>

@foreach ($members as $member)
<form id="member-remove-{{ (int) $member['id'] }}" method="POST" action="/admin/equipes/{{ (int) $team['id'] }}/members/{{ (int) $member['id'] }}/remove">@csrf</form>
@endforeach
